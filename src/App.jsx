...
export default function App() {
  ...
  const revisarAceptadasSinCargar=async()=>{
    try{
      const r=await fetch("/api/oc?listar=aceptadas&dias=30");
      if(!r.ok) return;
      const j=await r.json();
      if(!j.ok) return;
      const norm=(v)=>String(v||"").toUpperCase().replace(/[^A-Z0-9]/g,"").replace(/^N(?=\d)/,"");
