  ...
  const renderDetalle=(etapa)=>{
    ...
    return (
      <div>
        {eventos.length===0&&(
          <div>
            {/* Factura marcada en OC pero sin evento detallado */}
            ...
            {etapa.key!=="factura"&&<div style={{fontSize:12,color:C.inkFaint,padding:"4px 0 8px"}}>Sin registros aún</div>}
...
// Formulario compacto para agregar link directamente en Compra
function MiniFormLink({ ocId, onGuardar, orden }) {
...
// ═══════════════════════════════════════════════
function FormIngresarCompra({ ocs, financiadores, vendedores, onSave, entidadesCatalogo, ocExistente }) {
...
export default function App() {
  ...
  const handleIngresarCompra=async(data)=>{
    ...
    if(data.esNueva){
      ...
      // OC existente: actualizar todos los datos + guardar productos
  ...
  const handleEliminarFactura=async(ocId, facturaId)=>{
    ...
    // Si era la única factura, volver estado a pendiente
